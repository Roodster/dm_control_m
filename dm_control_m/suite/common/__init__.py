...
def read_model(model_filename):
    ...
    temp = os.path.join(_SUITE_DIR, model_filename)

    # If the path already exists than we get the file from the suite directory.
    if os.path.exists(temp):  
        return resources.GetResource(temp)
    # Otherwise we load the file from a  custom directory.
    else: 
